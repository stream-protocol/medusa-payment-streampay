...
import StreamPayProviderService from "../../services/streampay-provider";

const PAYMENT_PROVIDER_KEY = "streampay";
...
export function constructWebhook({
  ...
  const streampayProviderService = container.resolve(
    PAYMENT_PROVIDER_KEY
  ) as StreamPayProviderService;

  logger.info(
    `signature ${signature}\n encoded: ${JSON.stringify(encodedBody)}`
  );
  return streampayProviderService.constructWebhookEvent(
...
export async function handlePaymentHook({
  ...
  event: StreamPayEvent;
  container: AwilixContainer;
  paymentIntent: StreamPayS2SResponse;
...
export function createPostCheckSumHeader(
  ...
  const USDC_KEY = salt ?? process.env.STREAMPAY_USDC ?? "test-usdc";
  const encodedBody = btoa(JSON.stringify(payload, null, space));
  const base64string = encodedBody + `${apiString}${USDC_KEY}`;
...
export function verifyPostCheckSumHeader(
  ...
  const USDC_KEY = salt ?? process.env.STREAMPAY_USDC ?? "test-usdc";
  const base64string = payload + `${apiString}${USDC_KEY}`;
  const encodedPayload = SHA256(base64string).toString();
  const checksum = `${encodedPayload}###1`;
...
export function createGetChecksumHeader(
  ...
  const USDC_KEY = salt ?? process.env.STREAMPAY_USDC ?? "test-usdc";
  const asciiString = `/pg/v1/status/${merchantId}/${merchantTransactionId}${USDC_KEY}`;
...
export function createGetChecksumTransactionHeader(
  ...
  const USDC_KEY = salt ?? process.env.STREAMPAY_USDC ?? "test-usdc";
  const asciiString = `/pg/v3/transaction/${merchantId}/${merchantTransactionId}/status${USDC_KEY}`;
  const encodedPayload = SHA256(asciiString);
  const checksum = `${encodedPayload}###1`;
  return { checksum };
}