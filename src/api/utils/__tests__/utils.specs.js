...
describe("Utils", () => {
    ...
    describe("buildError", () => {
        it("should return the appropriate error message", () => {
            ...
            as StreamPay.StreamPayRawError

            let message = buildError(event, error)
            expect(message).toBe(
                `StreamPay webhook ${event} handle failed. This can happen when this webhook is triggered during a cart completion and can be ignored. This event should be retried automatically.${EOL}${error.detail}`
            )
            ...
            expect(message).toBe(
                `StreamPay webhook ${event} handle failed.${EOL}${error.detail}`
            )
            ...
            message = buildError(event, error)
            expect(message).toBe(
                `StreamPay webhook ${event} handling failed${EOL}${error.detail}`
            )
        })
    })
            ...
            describe("in a payment collection context", () => {
                it("should capture the payment collection if not already captured", async() => {
                    ...
                    expect(paymentCollectionService.retrieve).toHaveBeenCalledWith(
                        paymentIntent.metadata.resource_id, { relations: ["payments"] }
                    )
